"""
AsyncGeminiWebEnhancementAdapter

Gemini + Google Search Grounding 기반 비동기 웹 강화 어댑터
"""
...
from ....domain.web_enhancement.ports.async_web_enhancement_port import AsyncWebEnhancementPort
...
from ...ai_model.adapters.async_gemini_chat_adapter import AsyncGeminiChatAdapter


class AsyncGeminiWebEnhancementAdapter(AsyncWebEnhancementPort):
    """
    Gemini + Google Search Grounding 기반 비동기 웹 강화 어댑터

    특징:
    - AsyncGemini 클라이언트 사용으로 진정한 비동기 처리
    ...
    Attributes:
        async_gemini_adapter: AsyncGemini Chat 어댑터
    ...
    def __init__(
        ...
        Args:
            ...
            max_tokens: 최대 토큰 수
        """
        self._async_gemini_adapter = AsyncGeminiChatAdapter(
            api_key=api_key,
    ...
    async def enhance_terms(
        ...
        Single-shot 프롬프트:
        1. Google Search Grounding으로 웹 검색
        2. 10개 언어 동시 번역
        3. 웹 출처 URL 수집
        ...
        # 3. 비동기 Gemini 호출
        response_result = await self._async_gemini_adapter.execute(request.unwrap())

        if not response_result.is_success():
            return Failure(f"AsyncGemini 호출 실패: {response_result.unwrap_error()}")
