"""
AsyncGPT4oWebEnhancementAdapter

GPT-4o + 웹서치 기반 비동기 웹 강화 어댑터
"""
...
from ...ai_model.adapters.async_openai_chat_adapter import AsyncOpenAIChatAdapter


class AsyncGPT4oWebEnhancementAdapter(AsyncWebEnhancementPort):
    """
    GPT-4o + 웹서치 기반 비동기 웹 강화 어댑터

    특징:
    - AsyncOpenAI 클라이언트 사용으로 진정한 비동기 처리
    ...
    Attributes:
        async_openai_adapter: AsyncOpenAI Chat 어댑터
    ...
    def __init__(
        ...
        Args:
            ...
            max_tokens: 최대 토큰 수
        """
        self._async_openai_adapter = AsyncOpenAIChatAdapter(
            api_key=api_key,
    ...
    async def enhance_terms(
        ...
        # 3. 비동기 GPT-4o 호출
        response_result = await self._async_openai_adapter.execute(request.unwrap())

        if not response_result.is_success():
            return Failure(f"AsyncGPT-4o 호출 실패: {response_result.unwrap_error()}")
